...
--#4  Бардык 2000-жылдан 2010-жылга чейинки чыгарылган машиналарды чыгарыныз
select * from cars where year_of_issue in (2000,2010);
--#5  Chevrolet брендинде канча машина бар экенин чыгарыныз
select count(cars.id)  from cars  where brand='Chevrolet';
--#6   Машиналардын чыгарылган жылдарынын орточо маанисин чыгарыныз
select sum(cars.year_of_issue) from cars;
--#7  finding sum of year of issue;
...
select count(cars.id) from cars where brand ='Mercedes-Benz';
-- #13 Эн кымбат жана эн арзан машинаны чыгарыныз
select cars.brand,cars.price from cars where price = (select min(price) from cars);
select cars.brand,cars.price from cars where price = (select max(price) from cars);
...
select * from cars order by  year_of_issue desc limit 15 offset 7;
--#17  1995 - жылдан 2005- ке чейинки машиналарадан башкаларын чыгарыныз
select * from cars where year_of_issue in (1995,2005);
--#18  Бир тустогу саны эн коп болгон машиналарды чыгарг;
select color, COUNT(*) as color_count  from cars  group by   color order by  color_count desc    ;
select *
from cars where color ='Khaki';